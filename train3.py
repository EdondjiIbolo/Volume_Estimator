...
class VolumeEstimator(nn.Module):
    def __init__(self):
        ...
        self.model.classifier = nn.Identity()  # Eliminar la última capa

        # Reducción de dimensiones antes de la concatenación con la escala
        self.pool = nn.AdaptiveAvgPool2d(1)  # Reduce el tamaño de los features
        self.flatten = nn.Flatten()  

        # Nueva capa para manejar la escala
        self.fc = nn.Sequential(
            nn.Linear(in_features + 1, 512),  # Combinar features + escala
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 1)  # Salida de volumen estimado
        )

    def forward(self, x, escala):
        x = self.model.forward_features(x)  # Extraer features de la imagen
        x = self.pool(x)  # Aplicar Pooling Adaptativo para reducir dimensiones
        x = self.flatten(x)  # Convertir en vector de características

        escala = escala.view(-1, 1)  # Ajusta la escala para concatenarla
        x = torch.cat((x, escala), dim=1)  # Concatenar características con la escala
...
criterion = nn.SmoothL1Loss()  # Cambio de MSELoss a SmoothL1Loss
...
# ===========================
torch.save({"model_state_dict": model.state_dict()}, "models/modelo_volumen.pth")
